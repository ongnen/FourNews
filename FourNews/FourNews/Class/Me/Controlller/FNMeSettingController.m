...
- (void)setTableViewHeader{
    ...
    _waveToll.inView = tableHeader;
    if (self.realPersent <= 0.1) {
        self.realPersent = 0.1;
    }

    // 水位上升动画
    [_waveToll raiseAnimationWithDuration:2.0 toPersent:self.realPersent target:nil action:nil completion:nil];
    // 底部波浪动画
    ...
    // 创建控制器跳转过程中依然显示的waveAimation
    UIView *standHeader = [[UIView alloc] init];
    self.standHeader = standHeader;
    standHeader.backgroundColor = [UIColor colorWithRed:68/255.0 green:190/255.0 blue:167/255.0 alpha:1];
    ...
    standPersntL.textAlignment = NSTextAlignmentCenter;
    standPersntL.textColor = [UIColor orangeColor];
    standPersntL.font = [UIFont fontWithName:@"TransformersSolidNormal" size:30];
    standPersntL.text = _cacheStr;
    // 水位上升动画
    [standWaveToll raiseAnimationWithDuration:2.0 toPersent:self.realPersent target:nil action:nil completion:nil];
    // 底部波浪动画
    [standHeader addWaveAnimationWithWaveHeight:50 alpha:0.3];
    [[UIApplication sharedApplication].keyWindow insertSubview:standHeader atIndex:0];
}
